...
int departure(chain_stack *S,chain_stack *B,linked_queue **Q)/*  leave */
{
    char* car_no = (char *)malloc(20 * sizeof(char));
    printf("\n\t\t\t\t\t VEHICLE DEPARTURE");
    printf("\n\t\t\t\t\t-------------------");
...
/*int history(linked_queue **Q)
{
    FILE *fptr;
	fptr=fopen("history.dat","a");
	for(struct queue*i=(*Q)->front;i!=NULL;i=i->next)
    {
	   fwrite(&i->data,sizeof(struct node),1,fptr);
	}

	struct node b;
	struct queue *data;
	while(fread(&b,sizeof(struct node),1,fptr))
    {
		data=chain_stack();
	    copy(&data->data,&b);
	    //enqueue(Q,data);
...
int waiting_list(chain_stack *S,linked_queue **Q)/*  Check the current condition of the parking lot */
{
    ...
    printf("\n\t\t\t\t\t-----------------");
     ...
     {
         time_now = now_time();
         parking_time = time_now - S->stack_array[i].initial_time;/*  Calculate the current stop time */
         h = parking_time/H;
         parking_time = parking_time%H;
         m = parking_time/M;
         s = parking_time%M;
